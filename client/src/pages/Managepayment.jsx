...
export default function Managepayment() {
  ...
  let cardnum = loction.state.user.card[0].cardnumber;
  let len = cardnum.length;
  let num =
    cardnum[len - 1] + cardnum[len - 2] + cardnum[len - 3] + cardnum[len - 4];
  console.log(loction.state.x);
  const handleSubmit = () =>{
    ...
    // .then((e) => {console.log(e)})
     .then((response) =>response.json().then((dataa) => {
      // console.log(dataa)
      if(dataa === "success"){
      navigate("/payment" , {state:loction.state})
      }else if(dataa === "fail"){
  ...
  return (
    <>
      ...
      <Link style={{ textDecoration: "none", color: "black" }} to="/acc" state={loction.state.x}>
        <div
          style={{
            marginLeft: "80px",
            marginTop: "20px",
      ...
      <Modal show={show} onHide={handleClose} centered >
        ...
        <h4 className="my-4">Enter your Current Password to Continue</h4>
        <input type="password" className="form-control my-3 mx-2" onChange={(e) => {
